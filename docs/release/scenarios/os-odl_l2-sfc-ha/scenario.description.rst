...
The *client* and *server* virtual machines need to be located in the same compute
nodes where the service functions (SFs) are placed. This is due to a limitation in OpenDaylight,
which only installs the traffic classifier in the compute nodes where the SFs are.
