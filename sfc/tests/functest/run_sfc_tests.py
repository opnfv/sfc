...
class SfcFunctest(testcase.OSGCTestCase):
    ...
    def __disable_heat_resource_finder_cache_apex(self,controllers):
        ...
        for controller in controllers:
            ...
            controller.run_cmd('sudo cp {0} /home/heat-admin/'.format(remote_heat_conf_etc))
            controller.run_cmd('sudo chmod 777 {0}'.format(remote_heat_conf_home))
            ...
            controller.put_file(local_heat_conf, remote_heat_conf_home)
            controller.run_cmd('sudo cp {0} /etc/heat/'.format(remote_heat_conf_home))
            logger.info("Restart heat-engine in {0}".format(controller.ip))
            controller.run_cmd('sudo /bin/systemctl restart openstack-heat-engine.service')
    ...
    def run(self):
        ...
        self.__disable_heat_resource_finder_cache(nodes,COMMON_CONFIG.installer_type)

        odl_ip, odl_port = sfc_utils.get_odl_ip_port(nodes,COMMON_CONFIG.installer_type)
