...
class SfcOneChainTwoServiceTC(Common):
    ...
    def run(self):
        ...
        ovs_logger, network, router, sg, image_creator, creators = vvfn.preparation_openstack(COMMON_CONFIG,
                                                                                              TESTCASE_CONFIG, logger,
                                                                                              supported_installers)

        compute_nodes, controller_nodes = vvfn.installer_deployment_nodes(COMMON_CONFIG)
        odl_ip, odl_port, deployment_handler = vvfn.installer_deployment_odl(COMMON_CONFIG)
        ...
        server_instance, server_creator, client_instance, client_creator = vvfn.prepare_server_client_elements(
            COMMON_CONFIG, image_creator, vnfs, topo_seed, network, sg)
        ...
        vnfs = ['testVNF1', 'testVNF2']
        vvfn.create_custom_av(COMMON_CONFIG, tacker_client, vnfs[0], 'test-vnfd1', 'test-vim', test_topology)
        vvfn.create_custom_av(COMMON_CONFIG, tacker_client, vnfs[1], 'test-vnfd2', 'test-vim', test_topology)

        vvfn.path_join(COMMON_CONFIG, TESTCASE_CONFIG, tacker_client, client_instance, client_creator, 'red',
                       'red_http')
        ...
        results, verdict_2 = vvfn.present_results_http(COMMON_CONFIG, client_floating_ip, server_instance, ovs_logger,
                                                       deployment_handler, compute_nodes)

        total_verdict = (verdict and verdict_2 and verdict_allowed_http)
        logger.info(total_verdict)

        if __name__ == '__main__':
            return results.compile_summary(), creators

        if __name__ == 'sfc.tests.functest.sfc_one_chain_two_service_functions':
            return results.compile_summary(), creators
