...
def main():
    ...
    vvfn = TestRun()
    ovs_logger, network, router, sg = vvfn.preparation_vnfd(COMMON_CONFIG, TESTCASE_CONFIG, logger, supported_installers)

    compute_nodes, controller_nodes = vvfn.installer_deployment_nodes(COMMON_CONFIG)
    odl_ip, odl_port, deploymentHandler = vvfn.installer_deployment(COMMON_CONFIG)
    ...
    testTopology, topo_seed = vvfn.prepare_extra_variables(COMMON_CONFIG, compute_nodes, controller_nodes, vnfs)
    compute_clients, controller_clients = vvfn.prepare_client_resources(COMMON_CONFIG, compute_nodes, controller_nodes)
    ...
    router = vvfn.prepare_router(TESTCASE_CONFIG)
    client_instance, client_creator = vvfn.prepare_client_elements(
        COMMON_CONFIG, TESTCASE_CONFIG, CLIENT, vnfs, topo_seed)
    server_instance, server_creator = vvfn.prepare_server_elements(
        COMMON_CONFIG, TESTCASE_CONFIG, SERVER, vnfs, topo_seed)
    ...
    vvfn1.create_custom_vnfd(COMMON_CONFIG, TESTCASE_CONFIG, tacker_client, 'red', 'test-vnfd1')
    ...
    vvfn2.create_custom_vnfd(COMMON_CONFIG, TESTCASE_CONFIG, tacker_client, 'blue', 'test-vnfd2')
    ...
    server_floating_ip, client_floating_ip = vvfn.custom_floating_ip(router, client_instance, client_creator, server_instance, server_creator)
    ...
    results = vvfn.present_results_http(COMMON_CONFIG, client_floating_ip, server_instance, ovs_logger, deploymentHandler, compute_nodes)
    ...
    results = vvfn.block_http_trafic(sf1_floating_ip, sf2_floating_ip, client_floating_ip, ovs_logger, controller_clients, server_ip, compute_nodes, results)
