...
class SfcOneChainTwoServiceTC(sfc_parent_function.SfcCommonTestCase):
    ...
    def run(self):
        ...
        ovs_logger, network, router, sg, image_creator, creators = self.\
            prepare_env(TESTCASE_CONFIG, logger, supported_installers)
        ...
        test_topology, topo_seed = self.prepare_topology(vnfs)
        compute_clients, controller_clients = \
            self.prepare_client_resources(compute_nodes, controller_nodes)
        tacker_client = os_sfc_utils.get_tacker_client()

        server_instance, server_creator, client_instance, client_creator =\
            self.prepare_server_client_vm(image_creator, vnfs, topo_seed,
                                          network, sg)
        ...
        vnfs = ['testVNF1', 'testVNF2']
        self.create_custom_av(tacker_client, vnfs[0], 'test-vnfd1', 'test-vim',
                              test_topology)
        self.create_custom_av(tacker_client, vnfs[1], 'test-vnfd2', 'test-vim',
                              test_topology)

        neutron_port = self.create_vnffg(TESTCASE_CONFIG, tacker_client,
                                         client_instance, client_creator,
                                         'red', 'red_http')
