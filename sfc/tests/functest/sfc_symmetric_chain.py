...
def _create_custom_flavor_from_config():
    ...
    if custom_flavor_id is None:
        ...
        sys.exit(1)
    return custom_flavor_id


def _create_sf_image_from_config():
...
def main():
    ...
    custom_flavor_id = _create_custom_flavor_from_config()
    ...
    image_id = _create_sf_image_from_config()
    ...
    server_ip, client_ip, sf = test_utils.get_floating_ips_2(
        nova_client,
        neutron_client)

    if not test_utils.check_ssh([sf]):
    ...
    test_utils.vxlan_firewall(sf, port=blocked_port)
    ...
    if not test_utils.is_http_blocked(server_ip, client_ip, allowed_port):
    ...
    logger.info("Test HTTP")
    if test_utils.is_http_blocked(server_ip, client_ip, blocked_port):
