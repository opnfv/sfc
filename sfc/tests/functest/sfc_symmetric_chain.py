...
class SfcSymmetricChain(sfc_parent_function.SfcCommonTestCase):
    ...
    def run(self):
        ...
        vnf_ip = os_sfc_utils.get_vnf_ip(self.tacker_client,
                                         vnf_id=self.vnf_id)
        sf_floating_ip = self.assign_floating_ip_sf(vnf_ip)

        fips = [self.client_floating_ip, self.server_floating_ip,
                sf_floating_ip]

        self.check_floating_ips(fips, sf_floating_ip, sf_floating_ip)

        self.start_services_in_vm(sf_floating_ip, sf_floating_ip)
