...
import sfc.lib.test_utils as odl_utils
...
def main():
    ...
    server_subnet = TESTCASE_CONFIG.subnet_cidr
    server_ip_prefix = (
            server_ip
            + '/'
            + str(ipaddress.ip_network(server_subnet).prefixlen))
    ...
    # TODO
    # Assuming eth0 and eth1 as the port pair interface names on the vnf
    # Find a way to not have this hardcoded.
    # FIXME
    # vxlan_tool will forward on the ingress port, but it is supposed
    # to do it on the other port of the pair. For now, this wont matter.
    # But this is an ODL SFC implementation detail that may change at any
    # time.
    ...
    test_utils.start_vxlan_tool(sf_floating_ip, block="80", interface='eth0')
    test_utils.start_vxlan_tool(sf_floating_ip, interface='eth1')
    ...
    if test_utils.is_http_blocked(client_floating_ip,
                                  server_ip,
                                  TESTCASE_CONFIG.source_port):
        results.add_to_summary(2, "PASS", "HTTP Forward blocked")
    else:
        error = ('\033[91mTEST 2 [FAILED] ==> HTTP WORKS\033[0m')
        logger.error(error)
        test_utils.capture_ovs_logs(
            ovs_logger, controller_clients, compute_clients, error)
        results.add_to_summary(2, "FAIL", "HTTP Forward not blocked")
    ...
    logger.info("Starting HTTP firewall on %s" % sf_floating_ip)
    test_utils.start_vxlan_tool(sf_floating_ip, interface='eth0')
    test_utils.start_vxlan_tool(sf_floating_ip, block="80", interface='eth1')

    logger.info("Test HTTP again blocking response on SF1")
    if test_utils.is_http_blocked(client_floating_ip,
                                  server_ip,
                                  TESTCASE_CONFIG.source_port):
        results.add_to_summary(2, "PASS", "HTTP Reverse blocked")
    else:
        error = ('\033[91mTEST 2 [FAILED] ==> HTTP WORKS\033[0m')
        logger.error(error)
        test_utils.capture_ovs_logs(
            ovs_logger, controller_clients, compute_clients, error)
        results.add_to_summary(2, "FAIL", "HTTP Reverse not blocked")
