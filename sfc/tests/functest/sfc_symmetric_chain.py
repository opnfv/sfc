...
def main():
    ...
    server_ip_prefix = (
            server_ip
            + '/'
            + ipaddress.ip_network(TESTCASE_CONFIG.subnet_cidr).prefixlen)
    ...
    if test_utils.is_http_blocked(client_floating_ip,
                                  server_ip,
                                  TESTCASE_CONFIG.source_port):
        results.add_to_summary(2, "PASS", "HTTP Blocked Forward Path")
    else:
    ...
    if test_utils.is_http_blocked(client_floating_ip,
                                  server_ip,
                                  TESTCASE_CONFIG.source_port):
        results.add_to_summary(2, "PASS", "HTTP Blocked Reverse Path")
    else:
        error = ('\033[91mTEST 2 [FAILED] ==> HTTP WORKS\033[0m')
        logger.error(error)
        test_utils.capture_ovs_logs(
    ...
    if not test_utils.is_http_blocked(client_floating_ip, server_ip):
        results.add_to_summary(2, "PASS", "HTTP works again")
    else:
        error = ('\033[91mTEST 1 [FAILED] ==> HTTP BLOCKED\033[0m')
        logger.error(error)
        test_utils.capture_ovs_logs(
