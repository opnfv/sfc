...
class SfcTwoChainsSSHandHTTP(Common):
    ...
    def run(self):
        ...
        server_instance, server_creator, client_instance, client_creator =\
            vvfn.prepare_server_client_elements(SERVER, image_creator,
                                                vnf_names, topo_seed, network,
                                                sg)
        ...
        vvfn.create_custom_av(tacker_client, vnf_names[0], 'test-vnfd1',
                              'test-vim', topo_seed)
        vvfn.create_custom_av(tacker_client, vnf_names[1], 'test-vnfd2',
                              'test-vim', topo_seed)

        vvfn.path_join(TESTCASE_CONFIG, tacker_client, client_instance, 'red',
                       'red_http')
        ...
        results = vvfn.present_results_ssh(server_instance, deployment_handler,
