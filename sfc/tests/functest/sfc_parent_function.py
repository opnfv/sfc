...
class SfcCommonTestCase(object):
    ...
    def prepare_env(self, testcase_config, supported_installers):
        """
        Prepare the testcase environment and the components
        that the test scenario is going to use later on.
        Input : testcase_config,  supported_installers
        Output : (Environment preparation)
        """
        self.deployment_handler = DeploymentFactory.get_handler(
        ...
        self.openstack_nodes = \
            (self.deployment_handler.get_nodes({'cluster': cluster})
             if self.cluster is not None
             else self.deployment_handler.get_nodes())

        self.compute_nodes = \
            [node for node in self.openstack_nodes
             if node.is_compute()]
        ...
        self.network, self.router = \
            openstack_sfc.create_network_infrastructure(
        ...
        self.sg = \
            openstack_sfc.create_security_group(testcase_config.secgroup_name)
        ...
        self.odl_ip, self.odl_port = \
            odl_utils.get_odl_ip_port(self.openstack_nodes)
    ...
    def create_custom_vnfd(self, test_case_name, vnfd_name):
        ...
        Input : test_case_name, vnfd_name
        Output : -
        """
    ...
    def create_custom_av(self, vnf_names, av_member1, av_member2):
        """
        Create custom 'av'
        Input : vnf_names, av_member1, av_member2
        Output : -
        """
    ...
    def prepare_server_client_vm(self, vnf_names):
        """
        ...
        self.controller_clients = \
            test_utils.get_ssh_clients(self.controller_nodes)
    ...
    def assign_floating_ip(self):
        ...
        self.client_floating_ip = openstack_sfc.\
            assign_floating_ip(self.router, self.client_instance,
                               self.client_creator)
        self.server_floating_ip = openstack_sfc.\
            assign_floating_ip(self.router, self.server_instance,
                               self.server_creator)

    def custom_sf_floating_ip(self):
        """
    ...
    def check_floating_ips(self, fips, sf1_floating_ip, sf2_floating_ip):
        ...
        Input : fips, sf1_floating_ip, sf2_floating_ip
        Output : -
        """
    ...
    def start_services_in_vm(self, sf1_floating_ip, sf2_floating_ip):
        """
        Start the HTTP server in the server VM and
        Start the vxlan tool in the SFs
        Input: sf1_floating_ip, sf2_floating_ip
        Output : -
        """
    ...
    def create_vnffg(self, testcase_config, vnf_name, conn_name):
        """
    ...
    def check_deletion(self):
        ...
        if not odl_utils.check_vnffg_deletion(self.odl_ip, self.odl_port,
                                              self.ovs_logger,
                                              [self.neutron_port],
