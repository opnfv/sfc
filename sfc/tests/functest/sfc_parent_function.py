...
class SfcCommonTestCase(object):
    ...
    def register_vnf_template(self, test_case_name, template_name):
        ...
        if COMMON_CONFIG.mano_component == 'tacker':
            create_custom_vnfd(test_case_name, template_name)
    ...
    def create_vnf(self, vnf_name, vnfd_name=None, vim_name=None):
        ...
        if COMMON_CONFIG.mano_component == 'tacker':
            os_sfc_utils.create_vnf_in_av_zone(
                self.tacker_client, vnf_name, av_member1, av_member2,
            ...
            openstack_sfc.create_instance(CLIENT, COMMON_CONFIG.flavor,
                                          self.vnf_image_creator, self.network,
                                          self.sg,
    ...
    def create_vnffg(self, testcase_config_name, vnffgd_name, vnffg_name, port=80, protocol='tcp'):
        ...
        if COMMON_CONFIG.mano_component == 'tacker':
            ...
            self.neutron_port = openstack_sfc.get_instance_port(self.client_instance,
                                                                self.client_creator)
            ...
            for vnf in self.vnfs:
                # Take the instance [1], creator is [0]
                vnf_instance = self.objects[vnf][1]
                vnf_creator = self.objects[vnf][0]
                neutron_port = openstack_sfc.get_instance_port(vnf_instance,
                                                               vnf_creator)
                #TODO For the symmetric testcase ingres != egress
                port_group = openstack_sfc.create_port_groups(neutron_port,
                                                              vm_instance)
            ...
            self.neutron_port = openstack_sfc.get_instance_port(self.client_instance,
                                                                self.client_creator)            
            openstack_sfc.create_chain(self.port_groups, self.neutron_port, port, protocol, vnffg_name)
