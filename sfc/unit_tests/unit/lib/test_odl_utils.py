...
class SfcOdlUtilsTesting(unittest.TestCase):
    ...
    def test_actual_rsps_in_compute(self, mock_ovs_log, mock_compile):
        ...
        compile_calls = [call(r'.+(tp_(?:src|dst)=[0-9]+).+'
                              r'load:(0x[0-9a-f]+)->NXM_NX_NSP\[0\.\.23\].+')]

        match_calls = [call('msg_1'), call('msg_2')]
        group_calls = [call(2), call(1)]

        mf_1 = Mock()
        mf_1.group.side_effect = ['msg_p_1', 'msg_p_2']
        mock_compile.return_value.match.side_effect = [mf_1, None]
        ...
        mf_1.group.assert_has_calls(group_calls)
        mock_compile.assert_has_calls(compile_calls)
    ...
    # @patch('functools.wraps', lambda func: func)
    @patch('sfc.lib.odl_utils.logger', autospec=True)
    ...
    # # @patch('sfc.lib.odl_utils.timethis', lambda func: func)
    ...
    # def test_wait_for_this_raised_exception(self,
    #                                         mock_get_tacker_items,
    #                                         mock_rsps_in_compute,
    #                                         mock_get_odl_items,
    #                                         mock_find_compute,
    #                                         mock_log,
    #                                         mock_sleep):
    ...
    #     log_calls = [call('Error when waiting for classification'
    #                       ' rules: ErrorMSG')]

    #     mock_find_compute.side_effect = [Exception('ErrorMSG'), 'compute']
    #     mock_rsps_in_compute.return_value = None

    #     # error waiting for classification rules
    ...
    #     ErrorMSG = 'RSPs not configured in ODL'
    ...
    def test_get_odl_ip_port(self, mock_join,
        ...
        calls = [call('pwd'),
                 call('sudo cp /etc/neutron/plugins/ml2/ml2_conf.ini /etc/'),
        ...
        n1.run_cmd.assert_has_calls(calls)
    ...
    def test_format_odl_resource_list_url(self, mock_plularize,
        ...
        result = odl_utils.format_odl_resource_list_url('odl_ip', 'odl_port',
                                                        'resource')
