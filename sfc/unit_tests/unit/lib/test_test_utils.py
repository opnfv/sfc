...
class SfcTestUtilsTesting(unittest.TestCase):
    ...
    @patch('sfc.lib.test_utils.subprocess', autospec=True)
    def test_run_cmd(self,
        ...
        mock_subprocess.Popen.return_value.communicate.return_value = ('output',
                                                                        'error')
        mock_subprocess.Popen.return_value.returncode = 0
        test_utils.run_cmd(cmd)
        mock_subprocess.Popen.assert_called_once_with(cmd,
                                                      shell=True,
    ...
    @patch('sfc.lib.test_utils.urllib', autospec=True)
    @patch('sfc.lib.test_utils.shutil', autospec=True)
    def test_download_url_without_exception(self, 
        ...
        url = 'mocked_/url'
        mock_urllib.urlopen.return_value = response
        self.assertTrue(test_utils.download_url(url, dest_path))
        mock_urllib.urlopen.assert_called_once_with(url)
        mock_open.assert_called_once_with('mocked_/dest_/path/url', 'wb')
        mock_shutil.copyfileobj.assert_called_once_with(response, 
    ...
    def test_download_url_with_exception(self,
            ...
            dest_path = 'mocked_/dest_/path'
            url = 'mocked_/url'
            mock_urllib.urlopen.side_effect =  Exception('HttpError')
            self.assertFalse(test_utils.download_url(url, dest_path))
            mock_urllib.urlopen.assert_called_once_with(url)
            mock_open.assert_not_called()
            mock_shutil.copyfileobj.assert_not_called()

    @patch('sfc.lib.test_utils.os', autospec=True)
    @patch('sfc.lib.test_utils.logger', autospec=True)
    ...
    def test_download_image_file_not_found(self,
        ...
        mock_os.path.basename.return_value = 'image_path'
        mock_os.path.dirname.return_value = 'mocked_'
    ...
    @patch('sfc.lib.test_utils.time', autospec=True)
    @patch('sfc.lib.test_utils.logger', autospec=True)
    ...
    @patch('sfc.lib.test_utils.logger', autospec=True)
    @patch('sfc.lib.test_utils.run_cmd_remote', autospec=True)
    def test_start_vxlan_tool_retruned_false(self,
                                             mock_run_cmd_remote,
                                             mock_log,
                                             mock_time):
        """
    ...
    @patch('sfc.lib.test_utils.logger', autospec=True)
    @patch('sfc.lib.test_utils.run_cmd_remote', autospec=True)
    def test_start_vxlan_tool(self,
                              mock_run_cmd_remote,
                              mock_log,
                              mock_time):
        """
        ...
        self.assertTrue(test_utils.start_vxlan_tool(self.ip))
        mock_run_cmd_remote.assert_has_calls(rcr_calls)
    ...
    def test_netcat(self,
        ...
        cmd = 'nc -z  -w 5 %s 1234' % dest_ip
        log_calls = [call('Running [%s] from [%s] returns [0]' % (cmd, self.ip))]
    ...
    def test_capture_ovs_logs(self,
        ...
        mock_time.strftime.return_value = 'date_time'
    ...
    def test_check_ssh_returned_false(self,
        ...
        log_calls = [call('Checking SSH connectivity to the SFs with ips %s'
                                                                % str(ips))]
