...
class SfcCleanupTesting(unittest.TestCase):
    ...
    def test_delete_openstack_objects_exception(self, mock_log1, mock_log2):
        ...
        info_log_call = [call('Deleting the openstack objects...')] 

        cleanup.delete_openstack_objects(mock_creator_objs_list)

        mock_log1.assert_has_calls(log_calls)
        mock_log2.assert_has_calls(info_log_call)
    ...
    @patch('sfc.lib.openstack_utils.OpenStackSFC', autospec=True)
    def test_cleanup_nsfc_objects(self, mock_obj):
        instance = mock_obj.return_value
        cleanup.cleanup_nsfc_objects()
        instance.delete_chain.assert_called_once()
        instance.delete_port_groups.assert_called_once()
    ...
    @patch('sfc.lib.cleanup.untracked_security_groups')
    ...
    def test_cleanup(self,
        ...
        cleanup.cleanup('mano', ['creator_one', 'creator_two'],
                         self.odl_ip,
                         self.odl_port)
