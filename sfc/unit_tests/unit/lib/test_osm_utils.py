...
class SfcOsmUtilsTesting(unittest.TestCase):
    ...
    def test_get_vnf_ip_returns_none(self, mock_vnf_list, mock_log):
        """
        Checks the proper functionality of get_vnf_ip function
        when ip-address for vnf is not found
        """
        mock_vnf_id = 'abcd'
        mock_vnf_data = [{'id': 'xyzx', 'ip-address': 'x.y.z.x'},
                         {'id': 'xxxx', 'ip-address': 'x.x.x.x'}]
        mock_vnf_list.return_value = mock_vnf_data
        result = osm_utils.get_vnf_ip(self.osm_client, mock_vnf_id)
        assert result is None
        mock_log.error.assert_not_called()

    @patch('sfc.lib.osm_utils.logger', autospec=True)
    @patch('sfc.lib.osm_utils.vnf_list', autospec=True)
    def test_get_vnf_ip_raises_exception(self, mock_vnf_list, mock_log):
        """
        Checks the proper functionality of get_vnf_ip function
    ...
    @patch('sfc.lib.osm_utils.logger', autospec=True)
    @patch('sfc.lib.osm_utils.vnf_list', autospec=True)
    def test_get_vnf_ip(self, mock_vnf_list, mock_log):
        """
        Checks the proper functionality of get_vnf_ip function
        """
        mock_vnf_id = 'xxxx'
    ...
    def test_ns_create_returns_none_one(self, mock_ready, mock_log):
        """
    ...
    @patch('sfc.lib.osm_utils.logger', autospec=True)
    @patch('sfc.lib.osm_utils.wait_for_ns_to_be_ready', autospec=True)
    def test_ns_create_returns_none_two(self, mock_ready, mock_log, mock_open):
        """
        Checks the proper functionality of ns_create function
    ...
    @patch('sfc.lib.osm_utils.logger', autospec=True)
    @patch('sfc.lib.osm_utils.ns_list', autospec=True, return_value=[])
    def test_wait_for_ns_to_be_ready_returns_none(self,
        ...
        mock_ns_id = 'nsd-xyz'
        self.osm_client.ns.list.return_value = []
        log_calls = [call("Wait for the NS instance to be ready..."),
                     call("No network service found!")]
    ...
    def test_wait_for_ns_to_be_ready_raises_exception(self,
        ...
        mock_ns_id = 'nsd-xyz'
        ErrorMSG = "ERROR in creating nsd-xyz at RO"
        self.osm_client.ns.list.side_effect = [[{'id': 'nsd-abc',
                                                 'detailed-status': 'done'},
                                                {'id': 'nsd-xyz',
                                                 'detailed-status': "Getting"
                                                                    "ready"}],
                                               [{'id': 'nsd-abc',
                                                 'detailed-status': 'done'},
                                                {'id': 'nsd-xyz',
    ...
    @patch('time.sleep', autospec=True)
    @patch('sfc.lib.osm_utils.logger', autospec=True)
    def test_wait_for_ns_to_be_ready(self, mock_log, mock_sleep):
        """
        Checks the proper functionality of wait_for_ns_to_be_ready function
        """
    ...
    @patch('sfc.lib.osm_utils.logger', autospec=True)
    def test_ns_delete_returns_none(self, mock_log):
        """
        ...
        mock_ns_name = 'test-ns'
        ErrorMSG = "Failed to delete NS instance: {}".format(mock_ns_name)
        self.osm_client.ns.delete.side_effect = Exception(ErrorMSG)
        log_calls = [call("Error [ns_delete(osm_client, {}, {})]: {}"
                          .format(mock_ns_name, False, ErrorMSG))]
        osm_utils.ns_delete(self.osm_client, mock_ns_name)
        self.osm_client.ns.delete.assert_called_once_with(
            mock_ns_name, force=False)
        mock_log.error.assert_has_calls(log_calls)

    @patch('sfc.lib.osm_utils.logger', autospec=True)
    def test_ns_delete(self, mock_log):
        """
        Checks the proper functionality of ns_delete function
        """

        mock_ns = 'test-ns'
        osm_utils.ns_delete(self.osm_client, mock_ns)
        self.osm_client.ns.delete.assert_called_once_with(
            mock_ns, force=False)
        mock_log.error.assert_not_called()
    ...
    def test_wait_for_ns_to_be_deleted(self):
        ...
        """
        mock_ns_id = 'ns-xyz'
        osm_utils.wait_for_ns_to_be_ready
