...
def vnf_ip(osm_client, vnf_id):
    """
    Return the ip of given vnf
    """
    try:
        vnfs = vnf_list(osm_client, verbose=True)
        for vnf in vnfs:
            if vnf['id'] == vnf_id:
...
def wait_for_ns_to_be_ready(osm_client, ns_id):
    ...
    """
    logger.info("Wait for the NS instance to be ready...")
    try:
        while True:
            nsi = ns_show(osm_client, ns_id)
            if nsi:
...
def ns_show(osm_client, ns_id):
    """
    get the complete info of a given network service
    """
    nss = ns_list(osm_client, verbose=True)
    for nsi in nss:
        if nsi['id'] == ns_id:
            return nsi
    logger.error("No such network service found!")
    return None
...
def ns_delete(osm_client, name, force=False):
    ...
    try:
        ...
        wait_for_ns_to_be_deleted(osm_client, name)
    except Exception as e:
...
def wait_for_ns_to_be_deleted(osm_client, ns_id):
    ...
    while True:
        ...
        if nsi:
            if 'ERROR' in nsi['detailed-status']:
                logger.info("force deleting {}".format(ns_id))
