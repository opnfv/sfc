...
class CommonConfig(object):
    ...
    def __init__(self):
        ...
        logger.info("Installer Type : %s", self.installer_type)
        ...
        if self.nodes_pod is not None:
            self.installer_type = 'noInstaller'
            ...
            self.installer_fields = test_utils.\
                fill_installer_dict(self.installer_type)
            self.installer_ip = env.get('INSTALLER_IP')
            self.installer_user = ft_utils.\
                get_parameter_from_yaml(self.installer_fields['user'],
                                        self.config_file)
