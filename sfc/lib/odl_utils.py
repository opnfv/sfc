...
def get_active_rsps_on_port(odl_ip, odl_port, neutron_port):
    ...
    for acl in acls['access-lists']['acl']:
        ...
        if src_port == neutron_port:
            reverse_path = False
        elif dst_port == neutron_port:
            reverse_path = True
        else:
            continue
        acl_match_key = ('source-port-range' if reverse_path
                         else 'destination-port-range')
        of_match_value = matches[acl_match_key]['lower-port']
        of_match_key = 'tp_src' if reverse_path else 'tp_dst'
        of_port_match = of_match_key + '=' + of_match_value
