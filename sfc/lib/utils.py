...
def filer_sffs(compute_nodes, testTopology, vnfs):
    computes_ids_to_check = []
    for vnf in vnfs:
        computes_ids_to_check.append(testTopology[vnf].split('.')[0].split('-')[1]) # we must still handle the nova case

    computes_sff = []
    for compute in compute_nodes:
        if compute.id in compute_ids_to_check:
            computes_sff.append(compute.ssh)
