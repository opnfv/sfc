...
def create_vnf_in_av_zone(tacker_client, vnf_name, vnfd_name, av_zone=None):
    if av_zone is None:
        param_file = 'vnfd-templates/test-vnfd-default-params.yaml'
        os_tacker.create_vnf(tacker_client,
                             vnf_name,
                             vnfd_name=vnfd_name,
                             param_file=param_file)
        return
    tmp_dir = mkdir_p(os.path.join(os.getcwd(), 'tmp/'))
    param_file = os.path.join(tmp_dir, 'param_{0}.yaml'.format(av_zone))
    data = {'zone': av_zone}
