...
class OpenStackSFC:
    ...
    def register_glance_image(self, name, url, img_format, public):
        image_exists = glance_utils.get_image(self.glance, image_name=name)
        if image_exists:
            logger.info("Image %s already exists" % name)
    ...
    def create_flavor(self, name, ram, disk, vcpus):
        flavor_exists = nova_utils.get_flavor_by_name(self.nova, name)
        if flavor_exists:
            logger.info("Flavor %s already exists" % name)
    ...
    def create_instance(self, vm_name, flavor_name, image, network, secgrp,
                        av_zone):
        ...
        instance_settings = VmInstanceConfig(name=vm_name, flavor=flavor_name,
                                             security_group_names=secgrp.name,
                                             port_settings=[port_settings],
                                             availability_zone=av_zone)

        instance_creator = OpenStackVmInstance(self.os_creds,
                                               instance_settings,
                                               image.name)
    ...
    def assign_floating_ips(self, router):
        '''
