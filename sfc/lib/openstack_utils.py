...
nova_utils, neutron_utils, glance_utils, heat_utils)
...
class OpenStackSFC:
    ...
    def assign_floating_ip(self, router, vm):
        '''
        Assign a floating ips to all the VMs
        '''
        name = vm.name + "-float"
        port_name = vm.ports[0]
        float_ip = FloatingIpConfig(name=name,
                                    port_name=port_name,
                                    router_name=router.name)
        ip = vm.add_floating_ip(float_ip)
    ...
    def assign_floating_ip_vnfs(self, router):
        ...
        fips = []
        for stack in stacks:
            servers = heat_utils.get_stack_servers(self.heat, self.nova, stack)
            sf_ creator = create_instance.generate_creator(self.os_creds,
                                                           servers[0],
                                                           self.image_settings)
            ip = assign_floating_ip(router, servers[0])
            fips.append(ip)
