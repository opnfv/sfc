...
class OpenStackSFC:
    ...
    def create_instance(self, vm_name, flavor_name, image_creator, network,
        ...
        port_settings = []
        for port in ports:
            port_settings.append(PortConfig(port, network_name=network.name))
    ...
    def create_port_groups(self, vnf_ports, vm_instance):
        ...
        if len(vnf_ports) == 1:
            ...
            raise Exception("Failed to create por pairs")
        port_pair_info = \
    ...
    def create_chain(self, port_groups, neutron_port, port, protocol,
                     vnffg_name, symmetrical, server_port=None, server_ip=None):
        '''
