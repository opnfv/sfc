...
class OpenStackSFC:
    ...
    def __init__(self):
        ...
        self.neutron = neutronclient.Client(self.get_neutron_client_version(),
    ...
    def create_instance(self, vm_name, flavor, image, network,
        ...
        instance = self.conn.compute.create_server(
            name=vm_name, image_id=image.id, flavor_id=flavor_obj.id,
            networks=[{"port": port_obj.id}], key_name=keypair.name)
    ...
    def get_av_zones(self):
        ...
        # host = self.conn.identity.find_service("nova")
        # print (host.hypervisor_hostname)
        # return ['nova::' + host.hypervisor_hostname]
    ...
    def delete_all_objects(self, creators, testcase_config):
        ...
        for creator in reversed(creators):
            try:
                ...
                if creator.name == testcase_config.router_name:
                    logger.info("Remove subnet from router")
                    self.conn.network.remove_interface_from_router(
                        creator.id, subnet_obj.id)
                    logger.info("Delete router")
                    self.conn.network.delete_router(creator)
                    time.sleep(1)
                else:
                    creator.delete(self.conn.session)
                    time.sleep(1)
            except Exception as e:
