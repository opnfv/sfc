...
class OpenStackSFC:
    ...
    def create_port_groups(self, vnf_port, vm_instance):
        ...
        port_pair_id = self.neutron.port_pair_create(port_pair)
        if not port_pair_id:
        ...
        port_pair_group['description'] = 'port pair group for ' + vm_instance.name
        port_pair_group['port_pairs'] = []
        port_pair_group['port_pairs'].append(port_pair_id)
        port_pair_group_id = \
            self.neutron.port_pair_group_create(port_pair_group)
        if not port_pair_group_id:
            LOG.warning("Chain creation failed due to port pair group "
                        "creation failed for vnf "
                        "%(vnf)s", {'vnf': vm_instance.name)

        return port_pair_group_id

    def create_chain(self, port_groups, neutron_port, port, protocol, vnffg_name)
        '''
        ...
        sfc_classifier_params = {'name': vnffg_name + '-classifier',
                                 ...
                                 'protocol': protocol}
        fc_id = self.neutron.flow_classifier_create(sfc_classifier_params) 
        ...
        return self.neutron.port_chain_create(port_chain)
