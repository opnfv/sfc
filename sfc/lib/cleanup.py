...
def delete_openstack_objects(creators, testcase_config):
    openstack_sfc = os_sfc_utils.OpenStackSFC()
    openstack_sfc.delete_all_objects(creators, testcase_config)


def cleanup(testcase_config, creators, mano, odl_ip=None, odl_port=None):
    cleanup_mano_objects(mano)
    delete_openstack_objects(creators, testcase_config)
