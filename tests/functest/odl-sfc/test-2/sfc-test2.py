...
# To be fixed by SFC-52 -> adding them as normal libs
REPO_PATH = os.path.join(os.environ['repos_dir'], 'sfc/')
sys.path.append(REPO_PATH + "tests/functest/odl-sfc/")
...
def vxlan_tool_stop(sf):
    cmd = "for i in $(pgrep vxlan_tool.py);do kill -9 $i;done"
    run_cmd_remote(sf, cmd)
...
def main():
    ...
    vxlan_firewall(sf2, port="80", block=False)
    logger.info("Starting vxlan_tool on %s" % sf1)
    vxlan_firewall(sf1, port="80", block=False)
